...
- **Sprint 5 — Inbox Real-Time Fix**: Replaced 10-second polling fallback with a persistent Socket.io context. Messages now appear instantaneously.
- **Sprint 5 — Chatwoot UX Upgrade**: Implemented inline "Private Notes" mode with a sleek toggle, locking mechanism, and yellow bubble styling to differentiate from customer replies. Also upgraded the Slash Commands menu with `backdrop-blur-xl` for a premium Chatwoot-style aesthetic.
- **Sprint 5 — Brand Channel Icons**: Replaced generic heroicons with official WhatsApp, Messenger, and Instagram SVG brand logos throughout the Inbox (ConversationList + ChatWindow header). Each channel now displays its authentic brand color.
- **Sprint 5 — Label & Badge Redesign**: Overhauled all conversation labels (lifecycle stage, custom labels, group, bot, assigned user) with a cleaner 3-row card layout, smaller pill badges, and proper visual hierarchy. Added Smart View tabs (All/Mine/Unassigned/Bot) to ConversationList header.
- **Sprint 5 — Chat Background Pattern**: Added subtle dot-grid pattern to the chat messages area for visual depth.
